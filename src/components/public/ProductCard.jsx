...
export default function ProductCard({ product }) {
  ...
  const renderStars = (rating = 0) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<FaStar key={`full-${i}`} className="text-[#FF9900]" size={13}  />);
    }
    if (hasHalfStar) {
      stars.push(<FaStarHalfAlt key="half" className="text-[#FF9900]" size={13} />);
    }
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<FaRegStar key={`empty-${i}`} className="text-sage-light opacity-40" size={13} />);
    }

    return stars;
  };

  // Reviews ko compact karne ka function (e.g. 15000 -> 15k)
  ...
  return (
    <div className="bg-white border border-cream-dark rounded-[16px] sm:rounded-[20px] overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col h-full group">
      
      {/* 🟢 TOP: Image Container (Height reduced to 150px for a compact look) */}
      <div className="relative w-full h-[150px] bg-transparent flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-grow bg-white border-t border-cream-dark/30">
        
        {/* Category Name */}
        <p className="text-[10px] sm:text-[11px] font-bold text-sage-light uppercase tracking-wider mb-1 line-clamp-1">
          {product.category?.name || "Category"}
        </p>
        
        {/* Title (🟢 Forced to exactly 1 line limit) */}
        <Link href={`/products/${product.slug}`}>
          <h3 className="text-[14px] sm:text-[15px] font-bold text-sage-dark leading-snug line-clamp-1 hover:text-sage transition-colors mb-1.5">
            {product.title}
          </h3>
        </Link>
        ...
        {/* Pricing (🟢 FIX: flex-wrap add kiya taake PKR ya bari values par line break ho jaye) */}
        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1 mb-3 mt-auto">
          <span className="text-lg sm:text-[20px] font-extrabold text-sage-dark tracking-tight leading-none">
            {formatPrice(product.price, currency, rates)}
          </span>
          {product.originalPrice && product.originalPrice > product.price && (
            <span className="text-[12px] font-semibold text-sage-light line-through leading-none">
              {formatPrice(product.originalPrice, currency, rates)}
            </span>
          )}
        </div>

        {/* Solid View Deal Button (Slimmer vertically) */}
        <Link 
          href={`/products/${product.slug}`} 
          className="w-full flex items-center justify-center gap-1.5 bg-sage hover:bg-sage-dark text-white py-2 rounded-[10px] text-[13px] font-bold transition-all shadow-sm"
        >
