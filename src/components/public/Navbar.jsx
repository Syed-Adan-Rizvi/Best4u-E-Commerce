...
export default function Navbar({ settings }) {
  ...
  // Sync search bar with URL query (taake search bar mein text ruka rahay)
  useEffect(() => {
    const urlQuery = searchParams.get("search");
    if (urlQuery && !showSuggestions) {
      setSearchQuery(urlQuery);
    } else if (!urlQuery && !showSuggestions) {
      setSearchQuery("");
    }
  }, [searchParams]);
  ...
  // 🟢 AMAZON STYLE: Click karne par keyword search bar mein jayega aur shop page khulega
  const handleSuggestionClick = (keyword) => {
    setSearchQuery(keyword); // Text ko bar mein set karega
    setShowSuggestions(false);
    setIsMobileMenuOpen(false);
    document.activeElement?.blur();
    ...
    <header className="sticky top-0 z-50 w-full bg-cream border-b border-cream-dark shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-16 lg:h-20 gap-4">
          ...
          <div className="hidden lg:flex flex-1 w-full max-w-4xl mx-4 xl:mx-10 relative" ref={searchRef}>
            ...
            {/* 🟢 AMAZON STYLE DESKTOP SUGGESTIONS */}
                  ...
                  <ul className="py-2">
                    {suggestions.map((item) => (
                      <li key={item._id}>
                        ...
                        >
                          <Search size={14} className="text-sage-light" />
                          <span className="text-sm font-medium text-sage-dark truncate">{item.title}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
        ...
        <div className="lg:hidden pb-3 relative" ref={mobileSearchRef}>
          <form onSubmit={handleSearchSubmit} className="relative w-full">
           ...
           {/* 🟢 AMAZON STYLE MOBILE SUGGESTIONS */}
           {showSuggestions && suggestions.length > 0 && (
              <div className="absolute left-0 right-0 mt-1 bg-white border border-cream-dark rounded-xl shadow-xl overflow-hidden z-50">
                {isSearching ? (
                  ...
                  <ul className="py-1">
                    {suggestions.map((item) => (
                      <li key={item._id}>
                        ...
                        >
                          <Search size={14} className="text-sage-light" />
                          <span className="text-xs font-medium text-sage-dark truncate">{item.title}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
...
// suggestion dropdown ko enter yah search button click kerny ky baad suggestion nhi ay gi
// File Path: src/components/public/Navbar.jsx
// "use client";
...
//   // 🟢 1. Search Box Click Outside Ref
//   const searchRef = useRef(null);
//   const mobileSearchRef = useRef(null);
...
//   // 🟢 2. Global Click Outside Logic (Search + Currency dono ke liye)
//   useEffect(() => {
//     function handleClickOutside(event) {
//       if (currencyRef.current && !currencyRef.current.contains(event.target)) {
//         setIsCurrencyOpen(false);
//       }
//       // Agar user search container ke bahar click kare, toh suggestions band kar do
//       if (
//         searchRef.current && !searchRef.current.contains(event.target) &&
//         mobileSearchRef.current && !mobileSearchRef.current.contains(event.target)
//       ) {
...
//           if (data.success) {
//             setSuggestions(data.suggestions);
//             // 🟢 3. Agar array khali hai toh pop-up na dikhaye
//             setShowSuggestions(data.suggestions.length > 0); 
//           }
//         } catch (error) {
//           console.error("Suggestion fetch failed");
...
//   // 🟢 4. The Enter/Submit Fix
//   const handleSearchSubmit = (e) => {
//     e.preventDefault();
//     if (searchQuery.trim()) {
//       setShowSuggestions(false); // Dropdown foran band hoga
//       setIsMobileMenuOpen(false); // Mobile menu band hoga
      
//       // Input box se cursor (focus) hatao taake wo dubara activate na ho jaye
//       document.activeElement?.blur(); 
...
//           {/* 🟢 5. Desktop Search bar wrapped in ref */}
//           <div className="hidden lg:flex flex-1 w-full max-w-4xl mx-4 xl:mx-10 relative" ref={searchRef}>
//             <form onSubmit={handleSearchSubmit} className="w-full relative">
//               <input 
//                 type="text" 
...
//                 onChange={(e) => setSearchQuery(e.target.value)}
//                 onFocus={() => searchQuery.length >= 2 && setShowSuggestions(true)}
//                 // onBlur yahan se hata diya kyunke click outside zyada reliable hai
//                 className="w-full pl-5 pr-12 py-3 rounded-full border border-cream-dark bg-white focus:outline-none focus:ring-2 focus:ring-sage focus:border-transparent text-sm text-sage-dark shadow-inner transition-all"
//               />
//               <button type="submit" className="absolute right-1.5 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-sage text-white rounded-full hover:bg-sage-dark transition-colors shadow-sm">
//                 <Search size={15} />
...
//         {/* 🟢 6. Mobile Search bar wrapped in ref */}
//         <div className="lg:hidden pb-3 relative" ref={mobileSearchRef}>
//           <form onSubmit={handleSearchSubmit} className="relative w-full">
//             <input 
//               type="text" 
