  ...
  const fetchTrending = useCallback(async (pageNum = 1, isLoadMore = false) => {
    ...
    try {
      const queryParams = new URLSearchParams({
        ...
        limit: 12,
        sort: "Trending Deals"
        // 🟢 REVERT: t: Date.now() hata diya
      });

      // 🟢 REVERT: cache: 'no-store' hata diya
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        {loading && page === 1 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-cream-dark border-dashed mt-4 shadow-sm">
            ...
            <p className="text-sage-dark text-xl font-bold mb-2">There is no such a trending products</p>
            <p className="text-sage-light text-center max-w-md px-4">
              Click on products from the shop to see them trend here!
            </p>
          </div>
        )}
...
//         limit: 12,
//         sort: "Trending Deals",
//         t: Date.now() // 🟢 100% CACHE BUSTER: Har dafa fresh data layega
//       });
